...
#if defined(_WINDOWS)
static HANDLE clear_inherit_flag(HANDLE h0) {
    HANDLE hr;
    if(DuplicateHandle(GetCurrentProcess(), h0, GetCurrentProcess(), &hr, 0, FALSE, 
	    DUPLICATE_SAME_ACCESS)) {
	CloseHandle(h0);
	return hr;
    } else {
	/* probably some warning... */
	return h0;
    }
}
#endif
...
XmlRpcSocket::accept(int fd)
{
  ...
  SOCKET h0=::accept(fd, (struct sockaddr*)&addr, &addrlen);
  return (int) clear_inherit_flag((HANDLE)h0);
#else
